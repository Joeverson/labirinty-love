...
let animation = {
    move: {
        ...
        directions: {
            left: () => {
                return animation.move.frame("left")
            },
            up: () => {
                return animation.move.frame("up")
            },
            down: () => {
                return animation.move.frame("down")
            },
            right: () => {
                return animation.move.frame("right")
            }
        },
        load: (positions, pixels) => {
            //atribuindo as posições num escopo maior
            animation.move.positions = positions

            //definindo tamanhos default para sprite
            animation.move.pixels = pixels

            return {
                default: animation.move.directions.down()
            }
        },
        frame: way => {
            //sounds
            ...
            if (_.isArray(animation.move.positions[way])) {
                //verifica qual animação chamar / se for maior que o total de posições ele zera, caso seja menor ele ++
                if (animation.move.stage[way] < (animation.move.positions[way].length-1)) {
                    animation.move.stage[way]++
                } else {
                    animation.move.stage[way] = 0
                }

                //pegando as informações para a geração do frame
                let frame = animation.move.positions[way][animation.move.stage[way]]

                //criando o frame para alteração
                return new PIXI.Rectangle(frame.x, frame.y, animation.move.pixels.w, animation.move.pixels.h)

            } else {
                //criando o frame para alteração
                return new PIXI.Rectangle(animation.move.positions[way].x, animation.move.positions[way].y, animation.move.pixels.w, animation.move.pixels.h)
            }
        }
    }
}
